...
export default function ProfilePage() {
    ...
    // โหลด user และเติมค่าที่อยู่เดิม
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                // ที่อยู่
                const parsed = parseAddressString(data.address || user.address || '');
                if (addressDetailRef.current) addressDetailRef.current.value = parsed.detail || '';

                // ตั้งค่า Address State ทั้งหมดในครั้งเดียว
    ...
    // Functions to handle address dropdown changes
    ...
    const amphoeList = addressData.find(p => p.province === province)?.amphoes.map(a => a.amphoe) || [];
    const tambonList = addressData.find(p => p.province === province)?.amphoes.find(a => a.amphoe === amphoe)?.tambons.map(t => t.tambon) || [];
    ...
    const handleImageChange = async (e) => {
        ...
        try {
            ...
            // ✅ เพิ่มบรรทัดนี้: อัปเดตข้อมูลใน AuthContext
            updateUser({
                ...user,
    ...
    const handleSave = async () => {
        try {
            ...
            const addressString = composeAddressString({
                detail: addressDetailRef.current?.value || '',
